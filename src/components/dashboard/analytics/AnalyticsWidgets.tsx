...
import { Activity, TrendingUp, Calendar, Clock, Target, Award, Zap, BookOpen } from 'lucide-react'
import type { 
  AnalyticsOverview, 
  HabitStreak, 
  AIInsight, 
...
export function OverviewWidget({ overview }: OverviewWidgetProps) {
  const weeklyChange = overview.weeklyTrends.length >= 2 
        ...
        {topStreaks.map((streak) => (
          <div key={streak.habitId} className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
              <p className="text-white font-medium truncate">{streak.habitTitle}</p>
...
export function MoodCorrelationWidget({ correlations, className = '' }: MoodCorrelationWidgetProps) {
  ...
  const getCorrelationText = (correlation: number) => {
    if (correlation >= 0.5) return { text: 'Strong Positive', color: 'text-green-400' }
    if (correlation >= 0.3) return { text: 'Moderate Positive', color: 'text-blue-400' }
    if (correlation <= -0.5) return { text: 'Strong Negative', color: 'text-red-400' }
    if (correlation <= -0.3) return { text: 'Moderate Negative', color: 'text-orange-400' }
      ...
      <div className="space-y-4">
        ...
        {strongCorrelations.length === 0 && (
          <div className="text-center py-4">
            <p className="text-gray-400 mb-2">No strong correlations found yet</p>
...
export function AIInsightsWidget({ insights, className = '' }: AIInsightsWidgetProps) {
  const recentInsights = insights
    .slice(0, 3) // Just get first 3 since we don't have generatedAt
  ...
  const getInsightIcon = (type: string) => {
    switch (type) {
      ...
      case 'mood': return <Activity className="w-4 h-4" />
  ...
  const getInsightColor = (type: string) => {
    switch (type) {
      ...
      case 'mood': return 'text-green-400'
      default: return 'text-gray-400'
    }
  }
          ...
          <div key={insight.id} className="border-l-4 border-blue-500 pl-4 py-2">
            ...
            <div className="flex items-center justify-between mt-2">
              <span className={`text-xs px-2 py-1 rounded ${
                insight.confidence >= 0.8 ? 'bg-green-900 text-green-400' :
                insight.confidence >= 0.6 ? 'bg-blue-900 text-blue-400' :
...
export function WeeklyTrendsWidget({ trends, className = '' }: WeeklyTrendsWidgetProps) {
  const recentTrends = trends.slice(-4) // Last 4 weeks
  const averageCompletion = trends.length > 0 
    ...
    <div className={`bg-gray-800 rounded-lg p-6 border border-gray-700 ${className}`}>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          <p className="text-2xl font-bold text-blue-400">{averageCompletion}%</p>
