...
export function useWisdom() {
  ...
  const fetchQuotes = useCallback(async (searchQuery?: string, category?: string) => {
    try {
  ...
  const addToFavorites = useCallback(async (quote: Quote) => {
    try {
  ...
  const removeFromFavorites = useCallback(async (quoteId: string) => {
    try {
      const response = await fetch('/api/wisdom/favorites', {
        method: 'DELETE',
        headers: {
  ...
  const isFavorited = useCallback((quoteId: string) => {
    return favorites.some(fav => fav.quoteId === quoteId)
  }, [favorites])

  // Track user activity
  const trackActivity = useCallback(async (action: string, category?: string) => {
    try {
