...
export interface IHabitLog
  extends Omit<HabitLog, '_id' | 'habitId' | 'userId'>,
    Document {
  _id: mongoose.Types.ObjectId
...
HabitSchema.pre('save', function (next) {
  ...
  if (this.frequency.type === 'daily') {
    ...
    this.frequency.type === 'weekly' &&
    (!this.frequency.daysOfWeek || this.frequency.daysOfWeek.length === 0)
  ) {
    return next(new Error('Weekly habits must specify days of week'))
  } else if (
    this.frequency.type === 'custom' &&
    (!this.frequency.daysOfWeek || this.frequency.daysOfWeek.length === 0)
  ) {
  ...
  if (
    ['count', 'duration', 'amount'].includes(this.target.type) &&
    !this.target.value
  ) {
    return next(
      new Error(`${this.target.type} habits must have a target value`)
    )
  }
...
HabitSchema.statics.findActiveByUser = function (
  userId: mongoose.Types.ObjectId
) {
  return this.find({
    userId,
