...
export const onboardingHabitSchema = z.object({
  ...
  customName: z
    .string()
    .max(100, 'Custom habit name too long')
    .transform(sanitizeHtml)
    .optional(),
  customEmoji: z.string().max(10, 'Emoji too long').optional(),
  reminderTime: z
    .string()
    .regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Invalid reminder time format')
    .default('9:00'),
...
export const habitQuerySchema = z
  .object({
...
export const createJournalEntrySchema = z.object({
  ...
  mood: z
    .number()
    .min(1, 'Mood must be 1-10')
    .max(10, 'Mood must be 1-10')
    .optional(),
  tags: z
