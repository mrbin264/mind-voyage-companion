...
async function calculateWeeklyTrends(
  ...
  const weeklyData = new Map<
    string,
    { completed: number; total: number; start: Date; end: Date }
  >()
...
async function calculateMoodCorrelations(
  userId: string,
  startDate: Date
  ...
  const habitTypes = new Map<
    string,
    { moods: number[]; completions: boolean[] }
  >()
...
async function calculateJournalAnalytics(
  ...
  const writingConsistency =
    daysBetween > 0 ? Math.round((totalEntries / daysBetween) * 100) : 0
  ...
  const moodEntries = entries.filter(
    e => e.mood !== undefined && e.mood !== null
  )
  ...
  const weeklyStats = {
    ...
    wordsThisWeek: weekEntries.reduce(
      (sum, entry) => sum + (entry.wordCount || 0),
      0
    ),
    averageMoodThisWeek:
      weekEntries.filter(e => e.mood).length > 0
        ? weekEntries
          ...
          weekEntries.filter(e => e.mood).length
        : 0,
  }
...
function generateAIInsights(
  ...
  if (recentTrends.length > 0) {
    const avgCompletion =
      recentTrends.reduce((sum, t) => sum + t.completionRate, 0) /
      recentTrends.length
    if (avgCompletion < 60) {
      insights.push({
        id: 'challenge_day_1',
        ...
        description:
          'Your completion rate has been below average recently. Consider easier goals or better scheduling.',
  ...
  try {
    ...
    const overallCompletionRate =
      weeklyTrends.length > 0
        ? Math.round(
            weeklyTrends.reduce((sum, w) => sum + w.completionRate, 0) /
              weeklyTrends.length
          )
        : 0
    ...
    const overview: AnalyticsOverview = {
      ...
      totalHabitsCompleted: weeklyTrends.reduce(
        (sum, w) => sum + w.completedHabits,
        0
      ),
